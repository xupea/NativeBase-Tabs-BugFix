...
const ScrollableTabBar = createReactClass({
  ...
  getInitialState() {
    ...
    this._updateFromUserInteraction = false;
    return {
  ...
  updateView(offset) {
    ...
    if (initialPage !== 0) {
      if (!this._updateFromUserInteraction) {
        if (position === 0) {
          position = 2;
        }
      }
  ...
  updateTabUnderline(position, pageOffset, tabCount) {
    ...
    if (position < tabCount - 1) {
      ...
      this.state._leftTabUnderline.setValue(
        newLineLeft + (newLineRight - newLineLeft - 30) / 2
      );
      this.state._widthTabUnderline.setValue(30);
    } else {
      this.state._leftTabUnderline.setValue(lineLeft + 30 / 2);
      this.state._widthTabUnderline.setValue(30);
    }
  },
    ...
    if (typeof name === "string") {
      return (
        <Button
          key={`${name}_${page}`}
          onPress={() => {
            this._updateFromUserInteraction = true;
            onPressHandler(page);
          }}
